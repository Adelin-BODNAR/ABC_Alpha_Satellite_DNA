...
struct Args {
    ...
    int substitution_model_choice; /**< Integer variable used to store the choice of substitution model provided by the user (Jukes-Cantor, HKY, F84). */
...
Args get_args(int argc, char **argv) {
    ...
    parsed_args.substitution_model_choice = 0 ; /**< Initializes the default substitution model choice at 0 for Jukes-Cantor. */

    parsed_args.seed = 42 ; /**< Initializes the default seed at 42. */
    ...
    const std::string usage = "Usage: \n"
                            ...
                            "\t--substitution_model_choice SUBSTITUTION_MODEL_CHOICE \n"
                            "\t\tArgument defining the model used to perform the substitutions \n"
                            "\t\tin the sequence of a monomer during the simulation \n"
                            "\t\t(0 : Jukes-Cantor 1969, 1 : Kimura 1980, 2 : Felsenstein 1984) \n\n"
    ...
    */
    const struct option long_options[] =
        {
          ...
          {"substitution_model_choice", required_argument, 0, 0},
            ...
            case 0 :
                ...
                switch (option_index){
                    ...
                    case 12 :
                        ...
                        //std::cout << "Substitution model choice: " << optarg << std::endl << std::endl;
                        parsed_args.substitution_model_choice = std::stoi(optarg);

                        if (parsed_args.substitution_model_choice < 0 || parsed_args.substitution_model_choice > 2) {
                            std::cout << "Value not allowed for substitution model choice. \n(Values allowed -> 0 : Jukes-Cantor 1969, 1 : Kimura 1980, 2 : Felsenstein 1984)" << std::endl << std::endl;
                            std::cout << usage.c_str();
                            exit(1);
                        }
    ...
    if (parsed_args.verbose > 0) {
        ...
        std::cout << "Substitution model choice: " << parsed_args.substitution_model_choice << std::endl;
...
*/
Output_data amplification_simulation (int max_size, double amplification_rate, double alpha_amplification_size, double beta_amplification_size, int min_amplification_size, int max_amplification_size, double alpha_HOR_order, double beta_HOR_order, int min_HOR_order, int max_HOR_order, bpp::Sequence ancestor_monomer, std::shared_ptr<const bpp::Alphabet> alpha, double substitution_rate, int substitution_model_choice, int seed, int verbose ) {
    ...
    std::map<char,std::array<int,4>> substitution_model_probs ;

    std::vector<char> nucleotides = {'A', 'C', 'G', 'T'};

    switch (substitution_model_choice){

        case (0):

            substitution_model_probs =  {   {'A', { 0,  1,  1,  1} }, 
                                            {'C', { 1,  0,  1,  1} }, 
                                            {'G', { 1,  1,  0,  1} }, 
                                            {'T', { 1,  1,  1,  0} }
                                        };
    ...
    //std::cout << (substitution_model_distribs['T']).probabilities()[0] << " " << (substitution_model_distribs['T']).probabilities()[1] << " " << (substitution_model_distribs['T']).probabilities()[2] << " " << (substitution_model_distribs['T']).probabilities()[3] << std::endl;
    ...
    while(outputs.monomers_dataset.size() < max_size) {
        ...
        double time_amplification_event = bpp::ExponentialDiscreteDistribution(1,  1/ (amplification_rate * outputs.monomers_dataset.size())).randC();
        ...
        bpp::ExponentialDiscreteDistribution mutation_distrib = bpp::ExponentialDiscreteDistribution(1,  1/(substitution_rate * outputs.monomers_dataset.size() * ancestor_monomer.toString().size()));
        double time_mutation_event = mutation_distrib.randC();

        while (mutation_time_passed + time_mutation_event <= amplification_time_passed) {


            double index_mutated_monomer = int(uniform_distrib.randC() * (outputs.monomers_dataset.size()));
            double index_mutated_nucleotide = int(uniform_distrib.randC() * (ancestor_monomer.toString().size()));
            ...
            *(currently_mutated_sequence.begin()+index_mutated_nucleotide) = nucleotides[(substitution_model_distribs[currently_mutated_sequence[index_mutated_nucleotide]])(random_generator)] ;
            ...
            time_mutation_event = mutation_distrib.randC();
        }
