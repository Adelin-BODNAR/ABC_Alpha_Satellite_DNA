...
* @param tree_ptr Pointer to the the variable used to store the pointer to the output tree
* @param monomers_dataset_ptr Pointer to the variable used to store the output list of monomers 
*
* @return void
*/
void amplification_simulation (int max_size, double amplification_rate, double alpha_amplification_size, double beta_amplification_size, int min_amplification_size, int max_amplification_size, double alpha_HOR_order, double beta_HOR_order, int min_HOR_order, int max_HOR_order, bpp::Sequence ancestor_monomer, std::shared_ptr<const bpp::Alphabet> alpha, double substitution_rate, double transition_transversion_ratio, int seed, int verbose, bpp::TreeTemplate<bpp::Node>** tree_ptr, std::list<bpp::Node*>* monomers_dataset_ptr ) {
    ...
    *tree_ptr = new bpp::TreeTemplate<bpp::Node>(root_ptr);
    ...
    *monomers_dataset_ptr = {root_ptr};
    ...
    while((*monomers_dataset_ptr).size() < max_size) {
        ...
        index_head_monomers_to_amplify = int(uniform_distrib(random_generator) * ((*monomers_dataset_ptr).size()-HOR_order+1));
        ...
        position_new_monomer = int(uniform_distrib(random_generator) * ((*monomers_dataset_ptr).size()+2)) ; //TODO : Maybe choose with a beta distribution for instance instead of uniform
        ...
        for (int amplification_counter = 0; amplification_counter < amplification_size +1; amplification_counter++){
            ...
            for (int HOR_counter = 0; HOR_counter < monomers_to_amplify.size(); HOR_counter++){
                ...
                /**< Inserts the new monomer in the monomers dataset before the position specified by insert_position_iterator. */
                    ...
                    (*monomers_dataset_ptr).insert(insert_position_iterator,new_monomer); 
                    monomers_counter++;
        ...
        while (mutation_time_passed + time_mutation_event <= amplification_time_passed) {
            ...
            double index_mutated_monomer = int(uniform_distrib(random_generator) * ((*monomers_dataset_ptr).size()));
        ...
        if (verbose > 0){
            ...
            monomers_dataset_iterator = (*monomers_dataset_ptr).begin() ;
            std::for_each (monomers_dataset_iterator, std::next(monomers_dataset_iterator,(*monomers_dataset_ptr).size()), [monomers_dataset_ptr](bpp::Node* m) { std::cout << m->getId() << "\t" << m->getName() << "\t" << (dynamic_cast<bpp::BppInteger*>(m->getNodeProperty("amplification_id")))->getValue() << "\t" << *(dynamic_cast<bpp::BppString*>(m->getNodeProperty("parent_name"))) << "\t" << *dynamic_cast<bpp::BppString*>(m->getNodeProperty("sequence")) << std::endl << std::endl << std::endl; });
            std::cout << std::endl;
        }
...
int main(int argc, char **argv) {
    ...
    std::for_each (monomers_dataset_iterator, std::next(monomers_dataset_iterator,monomers_dataset.size()), [&monomers_dataset, &monomers_dataset_output_stream](bpp::Node* m) { monomers_dataset_output_stream << ">" << m->getName() << " " << "A_" << (dynamic_cast<bpp::BppInteger*>(m->getNodeProperty("amplification_id")))->getValue() << "\n" << *dynamic_cast<bpp::BppString*>(m->getNodeProperty("sequence")) << std::endl; });
